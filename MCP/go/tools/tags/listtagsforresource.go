...
func CreateListtagsforresourceTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_tags_resourceArn",
		mcp.WithDescription("Lists all tags on a AWS RoboMaker resource."),
		mcp.WithString("resourceArn", mcp.Required(), mcp.Description("The AWS RoboMaker Amazon Resource Name (ARN) with tags to be listed.")),
	)
