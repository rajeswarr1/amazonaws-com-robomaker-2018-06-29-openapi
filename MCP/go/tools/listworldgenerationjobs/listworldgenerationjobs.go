...
func ListworldgenerationjobsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/listWorldGenerationJobs%s", cfg.BaseURL, queryString)
		...
		var result models.ListWorldGenerationJobsResponse
...
func CreateListworldgenerationjobsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_listWorldGenerationJobs",
		...
		mcp.WithArray("filters", mcp.Description("Input parameter: Optional filters to limit results. You can use <code>status</code> and <code>templateId</code>.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    ListworldgenerationjobsHandler(cfg),
	}
}
