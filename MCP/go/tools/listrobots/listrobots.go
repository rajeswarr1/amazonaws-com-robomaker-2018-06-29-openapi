...
func ListrobotsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/listRobots%s", cfg.BaseURL, queryString)
		...
		var result models.ListRobotsResponse
...
func CreateListrobotsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_listRobots",
		mcp.WithDescription("<p>Returns a list of robots. You can optionally provide filters to retrieve specific robots.</p> <important> <p>This API will no longer be supported as of May 2, 2022. Use it to remove resources that were created for Deployment Service.</p> </important>"),
		mcp.WithString("maxResults", mcp.Description("Pagination limit")),
		mcp.WithString("nextToken", mcp.Description("Pagination token")),
		mcp.WithNumber("maxResults", mcp.Description("Input parameter: When this parameter is used, <code>ListRobots</code> only returns <code>maxResults</code> results in a single page along with a <code>nextToken</code> response element. The remaining results of the initial request can be seen by sending another <code>ListRobots</code> request with the returned <code>nextToken</code> value. This value can be between 1 and 200. If this parameter is not used, then <code>ListRobots</code> returns up to 200 results and a <code>nextToken</code> value if applicable. ")),
